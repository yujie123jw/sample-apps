...
<script runat="server">
    ...
    private DataTable GetTemplateInfo()
    {
        DataTable table = GenerateDataTable("Apcera Template Information");
		
		Assign(table, "uuid", "{{uuid}}");
		Assign(table, "name", "{{name}}");
		Assign(table, "num_instances", "{{num_instances}}");
		Assign(table, "cpu", "{{cpu}}");
		Assign(table, "memory", "{{memory}}");
		Assign(table, "disk", "{{disk}}");

		Assign(table, "Tags", "");

		{{range tags}}
			Assign(table, "Tag-->{{.Name}}", "{{.Value}}");
		{{end}}

		Assign(table, "Routes", "");
		{{range routes}}
			Assign(table, "Exposed URL", "{{.URL}}");
		
			{{if .Public}}
				Assign(table, "Listen Port", "{{.ListenPort}}");

			{{else}}
				Assign(table, "UpdatePort", "{{.ListenPort}}");
			{{end}}
		{{end}}

		Assign(table, "Bindings", "");
		{{range bindings}}
			Assign(table, "Name", "{{.Name}}");
			Assign(table, "Provider", "{{.Provider}}");
			Assign(table, "Service", "{{.Service}}");
			Assign(table, "URI", "{{.URI}}");
			Assign(table, "scheme", "{{.URI.Scheme}}");
			Assign(table, "user", "{{.URI.User}}");
			Assign(table, "password", "{{.URI.Password}}");
			Assign(table, "host", "{{.URI.Host}}");
			Assign(table, "port", "{{.URI.Port}}");
			Assign(table, "path", "{{.URI.Path}}");
			Assign(table, "raw_query", "{{.URI.RawQuery}}");
		{{end}}
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		LoadInformation(GetTemplateInfo());
